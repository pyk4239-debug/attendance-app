...
const COL_INSURANCE_SLIPS = "insurance_slips"; // 직원별 4대보험 내역(본인+회사부담) 전달용
...
const APP_VERSION = "v2026.07.13-r32";
...
const AGREEMENT_TEMPLATES = [
  ...
  { key: "cctv", label: "CCTV 촬영 동의서",
    content: `본인은 사업장 내 설치된 영상정보처리기기(CCTV)의 촬영에 대해 아래와 같이 안내받았으며 이에 동의합니다.\n\n1. 설치 목적: 시설 안전관리, 화재·도난 등 사고 예방, 안전사고 원인 확인\n2. 촬영 범위: 사업장 내 작업구역, 통로 등 주요 구역 (음성은 녹음되지 않음)\n3. 보관 기간: 촬영일로부터 30일 (사고·분쟁 등 필요 시 예외적으로 연장 보관될 수 있음)\n4. 열람 및 제공: 원칙적으로 안전관리 목적으로만 열람되며, 법령에 따른 경우를 제외하고 외부에 제공되지 않습니다.\n5. 촬영된 영상은 안전관리 및 사고 확인 목적 외의 용도(인사평가 등)로 사용되지 않습니다.\n\n본인은 위 내용을 안내받았으며, 사업장 내 CCTV 촬영에 동의합니다.` },
...
function AppLoader() {
  ...
  const [insuranceSlips, setInsuranceSlips] = useState([]);
  ...
  useEffect(() => {
    ...
    // 4대보험 개인별 내역
    unsubs.push(onSnapshot(query(collection(db, COL_INSURANCE_SLIPS), orderBy("createdAt", "desc")), snap => {
      setInsuranceSlips(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    }));
  ...
  return <App users={users} settings={settings} records={records} leaves={leaves}
    notices={notices} board={board} payslips={payslips} insuranceSlips={insuranceSlips} annual={annual} leaveRequests={leaveRequests} memberInfo={memberInfo} reads={reads}
...
// ── 팀원 개인 일정 캘린더 ──────────────────────────────────────────
function MemberScheduleCalendar({ settings = {}, scheduleEvents = [], leaveRequests = [], userId }) {
  ...
  // 승인된 연차 표기
  leaveRequests
    .filter(r => r.userId === userId && r.status === "승인" && r.date?.startsWith(selectedMonth))
    .forEach(r => {
      if (!eventMap[r.date]) eventMap[r.date] = [];
      eventMap[r.date].push({ type: "leave", label: `🏖 ${r.type}${r.type === "시간연차" && r.hours ? ` ${r.hours}시간` : ""}`, color: "#16a34a", id: r.id });
    });
  ...
  return (
    <div style={{ paddingBottom: 140 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, justifyContent: "flex-end" }}>
        {[["#dc2626","공휴일"],["#16a34a","연차"],["#0891b2","내 일정"]].map(([color,label]) => (
              ...
              <div key={i}>
                {editMode === "edit" && editTarget?.id === ev.id ? (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 0",
                    ...
                    {ev.type === "leave" && <span style={{ fontSize: 10, color: "#16a34a", fontWeight: 700, background: "#dcfce7", borderRadius: 6, padding: "2px 7px" }}>승인된 연차</span>}
...
function CalEventLabels({ events }) {
  const ORDER = { holiday: 0, leave: 1, reminder: 2, event: 3 };
...
function EditRecordModal({ user, date, rec, settings, userLeaves, onSave, onClose }) {
  ...
  // 반차 시 지각/조퇴 기준시각 자동 보정 (오전반차→점심종료 기준, 오후반차→점심시작 기준)
  const lm = calcLateMinWithLeave(inIso, settings.workStart, { type: leaveType }, settings);
  const em = calcEarlyOutMinWithLeave(outIso, settings.workEnd, inIso, settings.workStart, { type: leaveType }, settings);
  const om = calcTotalOvertimeMin(inIso, outIso, settings.workStart, settings.workEnd);
  const autoLate = lm > 0;
  const autoEarly = em > 0;
  const autoOvertime = om >= 30;
...
function WageModal({ user, info, monthStats, yearMonth, existing, holidays, annualData, settings, onClose, onSave }) {
  ...
  const bonusRate = Number(info?.bonusRate || 0);
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ padding: "5px 0", borderBottom: `1px solid ${T.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: T.text, whiteSpace: "nowrap", cursor: bonusRate > 0 ? "pointer" : "default" }}>
                  <input type="checkbox" checked={form.bonusEnabled} disabled={bonusRate === 0}
                    onChange={e => {
                      const on = e.target.checked;
                      setForm(p => ({ ...p, bonusEnabled: on, bonus: on ? Math.round(monthlyBase * (bonusRate / 100)) : 0 }));
                    }} />
                  상여금 지급
                </label>
                <input type="number" value={form.bonus} onChange={e => setForm(p => ({ ...p, bonus: e.target.value }))} style={{ ...iStyle, width: 130 }} placeholder="0" />
              </div>
              <div style={{ fontSize: 10, color: T.muted, marginTop: 3, textAlign: "right" }}>
                {bonusRate > 0 ? `등록된 상여율 ${bonusRate}% (기본급 × ${bonusRate}% = ${Math.round(monthlyBase * (bonusRate / 100)).toLocaleString()}원)` : "상여율 미등록 — 팀원 기초데이터에서 등록해주세요"}
              </div>
            </div>
            {[["이월분", "carryOver"], ["기타", "otherIncome"]].map(([label, key]) => (
...
// ── 관리자 임금 섹션 ──────────────────────────────────────────
function AdminWage({ users, records, leaves, settings, memberInfo, annual, leaveRequests, payslips, insuranceSlips = [], reads, onBack }) {
  const members = users.filter(u => u.role === "member" && (!u.status || u.status === "active"));
  const kst = new Date(new Date().getTime() + 9 * 60 * 60 * 1000);
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Noto Sans KR',sans-serif" }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        {tab === "payslip" && (
          <PayslipScreen user={users.find(u => u.role === "admin") || users[0]} users={users} payslips={payslips || []} insuranceSlips={insuranceSlips || []} reads={reads || {}} />
        )}
      </div>
...
function MemberInfoModal({ user, info, onSave, onClose }) {
  ...
  const [d, setD] = useState({
    ...
    bonusRate: "",        // 상여율(%) — 기본급 대비 비율, 지급 시 자동계산용
    ...info
  });
  const iStyle = { width: "100%", padding: "11px 14px", borderRadius: 10, border: `1px solid ${T.border}`, background: "#fff", color: T.text, fontSize: 14, fontWeight: 600, boxSizing: "border-box", fontFamily: "inherit" };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000066", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
      <div style={{ background: T.card, borderRadius: 20, padding: 22, width: "100%", maxWidth: 340, maxHeight: "90vh", overflowY: "scroll", WebkitOverflowScrolling: "touch", boxShadow: "0 20px 60px #00000030" }}>
        ...
        {numField("상여율 (%)", "bonusRate", "0", "기본급 대비 비율 · 급여 계산 시 \"상여금 지급\" 체크하면 자동계산")}
...
// ── 일정 (캘린더 + 리마인더) ─────────────────────────────────────
function AdminSchedule({ reminders = [], users = [], settings = {}, scheduleEvents = [], leaveRequests = [] }) {
...
// ── 일정 캘린더 뷰 ───────────────────────────────────────────────
function ScheduleCalendar({ reminders = [], settings = {}, scheduleEvents = [], leaveRequests = [], users = [], onSwitchToReminder }) {
  const kstNow = new Date(new Date().getTime() + 9 * 60 * 60 * 1000);
  const currentMonth = kstNow.toISOString().slice(0, 7);
  const [selectedMonth, setSelectedMonth] = useState(currentMonth);
  const [selDate, setSelDate] = useState(null);
  ...
  // 4) 승인된 연차 (전 직원)
  leaveRequests
    .filter(r => r.status === "승인" && r.date?.startsWith(selectedMonth))
    .forEach(r => {
      if (!eventMap[r.date]) eventMap[r.date] = [];
      const uname = users.find(u => u.id === r.userId)?.name || "팀원";
      eventMap[r.date].push({ type: "leave", label: `🏖 ${uname} ${r.type}${r.type === "시간연차" && r.hours ? ` ${r.hours}시간` : ""}`, color: "#16a34a", id: r.id });
    });
  ...
  return (
    <div style={{ padding: 16, paddingBottom: 140 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, justifyContent: "flex-end", flexWrap: "wrap" }}>
        {[["#dc2626","공휴일"],["#16a34a","연차"],["#7c3aed","리마인더"],["#0891b2","일정"]].map(([color,label]) => (
          <div key={label} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 700 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: color }} />
            <span style={{ color: T.muted }}>{label}</span>
          </div>
              ...
              <div style={{ background: T.card, borderRadius: 16, padding: 16, border: `1px solid ${T.border}` }}>
                ...
                {OTHER_TEMPLATES.map(t => (
                  <button key={t.key} onClick={() => {
                    ...
                    if (t.key === "dismissal") {
                      const today = new Date();
                      const fmtDot3 = (d) => new Date(d).toLocaleDateString("ko-KR", { year: "numeric", month: "long", day: "numeric" });
                      const content = `본 통지서는 근로기준법 제27조에 따라 해고사유와 해고시기를 서면으로 통지하기 위해 작성되었습니다.\n\n■ 본 통지서 도달일: ${fmtDot3(today)}\n■ 해고 예정일(효력 발생일): 년 월 일\n\n■ 해고 사유:\n(구체적인 사실관계와 사유를 기재해주세요)\n\n■ 해고예고 (근로기준법 제26조)\n사용자는 근로자를 해고하려면 해고 예정일 30일 전에 예고해야 하며, 예고하지 않은 경우 30일분 이상의 통상임금(해고예고수당)을 지급해야 합니다. 다만 아래의 경우에는 예고 없이 해고할 수 있습니다.\n1. 근로자가 계속 근로한 기간이 3개월 미만인 경우\n2. 천재사변, 그 밖의 부득이한 사유로 사업을 계속하는 것이 불가능한 경우\n3. 근로자가 고의로 사업에 막대한 지장을 초래하거나 재산상 손해를 끼친 경우\n\n위 사유로 상기 일자부로 근로계약을 해지함을 통지합니다.`;
                      setForm(p => ({ ...p, docTitle: "해고통지서", docContent: content, templateKey: "dismissal" }));
                      return;
                    }
...
■ 근거: 산업안전보건법 제29조`,
  ...
  {
    key: "newhire",
    label: "신규 채용자 안전보건교육 (8시간)",
    getTitle: () => `신규 채용자 안전보건교육`,
    content: `■ 교육 종류: 신규 채용자 안전보건교육 (채용 시 교육)
■ 교육 일시: {{DATETIME}}
■ 교육 시간: 8시간 이상
■ 교육 내용:
  1. 산업안전보건법령 및 산업재해보상보험 제도에 관한 사항
  2. 작업공정의 유해·위험요인과 안전보건조치에 관한 사항
  3. 물질안전보건자료(MSDS)에 관한 사항 (해당 시)
  4. 위험성평가에 관한 사항
  5. 보호구 착용 및 정리정돈에 관한 사항
  6. 응급조치 및 사고 발생 시 대응요령에 관한 사항
■ 근거: 산업안전보건법 시행규칙 제26조 (채용 시 교육)
■ 대상: 아래 수신인 중 신규 입사자만 선택해주세요 (전체 발송 금지)`,
  },
];
...
function EducationSection({ users, reads, onBack }) {
  ...
  const applyTemplate = (key) => {
    ...
    // 신규채용자 교육은 전체가 아닌 해당 입사자 1인만 받아야 하므로 수신자를 비워 직접 선택하도록 함
    if (key === "newhire") setRecipients([]);
  };
...
function InsuranceSection({ users, memberInfo, settings, onBack }) {
  ...
  // 팀원 한 명에게 4대보험 내역 전송 (개인별 카드 버튼 / 일괄전송 둘 다 재사용)
  const sendOneSlip = async (r) => {
    const memberId = r.id;
    if (!memberId) { alert("과거 저장본이라 식별정보가 없어요. 계산하기를 다시 눌러 최신화한 뒤 재전송해주세요."); return false; }
    await setDoc(doc(db, COL_INSURANCE_SLIPS, `${memberId}_${monthKey}`), {
      userId: memberId, month: monthKey, name: r.name,
      pension: r.pension, health: r.health, rows: r.rows,
      합계_근로자: r.합계_근로자, 합계_사업주: r.합계_사업주,
      createdAt: new Date().toISOString(),
    });
    await addDoc(collection(db, COL_NOTICES), {
      title: `🧾 ${monthKey} 4대보험 내역 안내`,
      content: `${monthKey} 4대보험 내역(본인부담 + 회사부담)이 등록되었습니다.\n급여명세서 탭 > 4대보험 내역에서 확인해주세요.`,
      recipient: memberId, author: "관리자", createdAt: new Date().toISOString(), auto: true,
    });
    await sendPush({ title: `🧾 4대보험 내역 안내`, message: `${monthKey} 4대보험 내역이 등록되었습니다.`, targetUserId: memberId });
    return true;
  };

  const sendSlipToOne = async (r) => {
    if (!window.confirm(`${r.name}님께 ${monthKey} 4대보험 내역을 전송할까요?\n(본인부담 + 회사부담 내역이 함께 표시됩니다)`)) return;
    setSendingSlipId(r.id || r.name);
    try {
      const ok = await sendOneSlip(r);
      if (ok) {
        setJustSentIds(p => ({ ...p, [r.id]: true }));
        setTimeout(() => setJustSentIds(p => { const n = { ...p }; delete n[r.id]; return n; }), 3000);
      }
    } catch (e) { alert("전송 실패: " + e.message); }
    setSendingSlipId(null);
  };

  const sendSlipsToMembers = async () => {
    if (!results) { alert("먼저 계산하기를 눌러주세요."); return; }
    const targets = results.all.filter(r => !r.isOwner);
    if (targets.length === 0) { alert("전송할 팀원이 없습니다."); return; }
    if (!window.confirm(`${monthKey} 4대보험 내역을 팀원 ${targets.length}명에게 전송할까요?\n(본인부담 + 회사부담 내역이 함께 표시됩니다)`)) return;
    ...
    try {
      ...
      alert(skipped > 0
        ? `${sent}명에게 전송했습니다. (${skipped}명은 과거 저장본이라 식별정보가 없어 건너뛰었어요 — 계산하기를 다시 눌러 최신화한 뒤 재전송해주세요.)`
        : `${sent}명에게 전송 완료됐습니다.`);
    } catch (e) { alert("전송 실패: " + e.message); }
    ...
    getDoc(doc(db, COL_INSURANCE, monthKey)).then(snap => {
      if (snap.exists()) {
        ...
        if (d.inputs) {
          ...
          if (d.inputs.memberInputs) {
            // 저장 이후 퇴사(삭제) 처리된 팀원은 이번 달 계산에서 제외하고,
            // 저장 이후 새로 등록된 활성 팀원은 빈 입력값으로 추가합니다.
            const activeIds = new Set(activeMembers.map(m => m.id));
            const savedIds = new Set(d.inputs.memberInputs.map(mi => mi.id));
            const reconciled = d.inputs.memberInputs.filter(mi => activeIds.has(mi.id));
            activeMembers.forEach(m => {
              if (!savedIds.has(m.id)) {
                const info = memberInfo[m.id] || {};
                const pension = Number(info.pensionBase) || 0;
                const health = Number(info.insuranceBase) || 0;
                reconciled.push({ id: m.id, name: m.name, pension: pension ? String(pension) : "", health: health ? String(health) : "" });
              }
            });
            const changed = reconciled.length !== d.inputs.memberInputs.length;
            setMemberInputs(reconciled);
            // 팀원 구성이 바뀌었으면 저장된 계산결과(합계)가 더 이상 정확하지 않으므로,
            // 결과는 비워서 다시 "계산하기"를 누르도록 유도합니다.
            if (changed) { setResults(null); setSavedThisMonth(false); } else if (d.results) setResults(d.results);
          } else if (d.results) {
  ...
  // 관리자 기초데이터 불러오기 — 이번 달 저장본이 이미 있으면 그 값이 우선이므로 스냅샷 조회가 끝난 뒤에만 폴백으로 사용
  useEffect(() => {
    if (!admin?.id) return;
    getDoc(doc(db, COL_INSURANCE, monthKey)).then(monthSnap => {
      const hasOwnerFromSnapshot = monthSnap.exists() && monthSnap.data()?.inputs?.ownerPension !== undefined;
      if (hasOwnerFromSnapshot) return; // 이번 달 저장된 관리자 보수월액이 있으면 기초데이터로 덮어쓰지 않음
      getDoc(doc(db, COL_MEMBER_INFO, admin.id)).then(snap => {
        if (snap.exists()) {
          const d = snap.data();
  ...
  const calculate = async () => {
    ...
    // 클라이언트에 캐시된 savedThisMonth는 화면 진입 직후 비동기 조회가 끝나기 전이면 아직 false일 수 있어
    // (그 사이 계산하기를 누르면 확인창 없이 덮어써지는 문제) — 저장 직전 Firestore를 한 번 더 직접 확인
    let alreadySaved = savedThisMonth;
    try {
      const snap = await getDoc(doc(db, COL_INSURANCE, monthKey));
      alreadySaved = snap.exists();
    } catch (e) { /* 조회 실패 시 캐시값으로 판단 */ }
    if (alreadySaved) {
    ...
    try {
      ...
      const memberResults = memberInputs.filter(m => m.pension || m.health).map(m => ({
        id: m.id, name: m.name, isOwner: false, pension: num(m.pension), health: num(m.health),
      ...
      }).then(() => setSavedThisMonth(true)).catch((e) => {
        alert("⚠ 계산 결과가 서버에 저장되지 않았습니다.\n화면에는 반영됐지만, 다시 들어오면 이전 값으로 보일 수 있습니다.\n\n오류: " + e.message + "\n\n다시 시도해주세요.");
      });
    } catch(e) {
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Noto Sans KR',sans-serif", paddingBottom: 40 }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ background: "#faf5ff", borderRadius: 14, padding: 14, marginBottom: 12, border: "2px solid #c4b5fd" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: "#7c3aed" }}>👑 관리자 (사업주)</div>
            <button onClick={async () => {
              if (!admin?.id) return;
              try {
                const snap = await getDoc(doc(db, COL_MEMBER_INFO, admin.id));
                if (!snap.exists()) { alert("등록된 기초데이터가 없습니다."); return; }
                const d = snap.data();
                const pension = Number(d.pensionBase) || 0;
                const health = Number(d.insuranceBase) || 0;
                if (!window.confirm(`현재 입력값을 프로필 기초데이터(기준소득월액 ${pension.toLocaleString()}원 · 보수월액 ${health.toLocaleString()}원)로 덮어쓸까요?`)) return;
                setOwnerPension(pension ? String(pension) : ownerPension);
                setOwnerHealth(health ? String(health) : ownerHealth);
              } catch (e) { alert("불러오기 실패: " + e.message); }
            }}
              style={{ fontSize: 10.5, fontWeight: 700, color: "#7c3aed", background: "none", border: "1px solid #c4b5fd", borderRadius: 8, padding: "4px 8px", cursor: "pointer" }}>
              🔄 기초데이터 새로고침
            </button>
          </div>
        ...
        <div style={{ background: T.card, borderRadius: 14, padding: 14, marginBottom: 16, border: `1px solid ${T.border}` }}>
          ...
          <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 10 }}>※ 최초 1회 기초데이터 자동 입력 · 이후엔 저장된 값 유지 (수정 가능). 프로필에서 기준소득월액을 바꿨다면 아래 새로고침 버튼으로 반영하세요.</div>
            ...
            <div key={m.id} style={{ background: T.bg, borderRadius: 12, padding: 12, marginBottom: 10, border: `1px solid ${T.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                ...
                <button onClick={() => {
                  ...
                  const pension = Number(info.pensionBase) || 0;
                  const health = Number(info.insuranceBase) || 0;
                  if (!pension && !health) { alert("등록된 기초데이터(기준소득월액)가 없습니다."); return; }
                  if (!window.confirm(`${m.name}님의 현재 입력값을 프로필 기초데이터(기준소득월액 ${pension.toLocaleString()}원 · 보수월액 ${health.toLocaleString()}원)로 덮어쓸까요?`)) return;
                  setMemberInputs(p => p.map((x, j) => j === i ? { ...x, pension: pension ? String(pension) : x.pension, health: health ? String(health) : x.health } : x));
                }}
                  style={{ fontSize: 10.5, fontWeight: 700, color: T.sub, background: "none", border: `1px solid ${T.border}`, borderRadius: 8, padding: "4px 8px", cursor: "pointer" }}>
                  🔄 기초데이터 새로고침
                </button>
              </div>
        ...
        {results && (<>
          ...
          {results.all.map((r, ri) => (
            <div key={ri} style={{ background: T.card, borderRadius: 14, padding: 14, marginBottom: 12, border: r.isOwner ? "2px solid #c4b5fd" : `1px solid ${T.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                ...
                {!r.isOwner && (
                  <button onClick={() => sendSlipToOne(r)} disabled={sendingSlipId === (r.id || r.name)}
                    style={{ flexShrink: 0, fontSize: 11, fontWeight: 700, padding: "6px 10px", borderRadius: 8, border: "none", cursor: sendingSlipId === (r.id || r.name) ? "default" : "pointer",
                      background: justSentIds[r.id] ? "#dcfce7" : (sendingSlipId === (r.id || r.name) ? "#e5e7eb" : "#0f766e"),
                      color: justSentIds[r.id] ? "#16a34a" : (sendingSlipId === (r.id || r.name) ? "#9ca3af" : "#fff") }}>
                    {justSentIds[r.id] ? "✓ 전송됨" : sendingSlipId === (r.id || r.name) ? "전송 중..." : "📤 전송"}
                  </button>
                )}
              </div>
          ...
          <button onClick={sendSlipsToMembers} disabled={sendingSlips}
            style={{ display: "block", width: "100%", padding: "13px 0", borderRadius: 14, border: "none", background: sendingSlips ? "#e5e7eb" : "#0f766e", color: sendingSlips ? "#9ca3af" : "#fff", fontSize: 14, fontWeight: 800, cursor: sendingSlips ? "default" : "pointer", marginBottom: 12 }}>
            {sendingSlips ? "전송 중..." : "👥 전체 팀원에게 일괄 전송"}
          </button>
...
// ── 관리자 화면 (라우터) ───────────────────────────────────────
function AdminScreen({ user, users, settings, records, leaves, notices, board, payslips, insuranceSlips = [], annual, leaveRequests, memberInfo, reads, reminders = [], scheduleEvents = [], contracts = [], notiLog = [], riskAssessments = [], riskSubmissions = [], onSaveRecord, onSaveLeave, onSaveUsers, onSaveSettings, onLogout }) {
  ...
  if (section === "wage") return <><AdminWage users={users} records={records} leaves={leaves} settings={settings} memberInfo={memberInfo} annual={annual} leaveRequests={leaveRequests} payslips={payslips} insuranceSlips={insuranceSlips} reads={reads} onBack={back} /><FloatBack onClick={back} /></>;
  ...
  if (section === "annual") return <><AnnualScreen user={user} users={users} annual={annual} leaveRequests={leaveRequests} memberInfo={memberInfo} onBack={back} /><FloatBack onClick={back} /></>;
  ...
  if (section === "schedule") return <AdminSectionWrap title="🗓 일정" color="#7c3aed" onBack={back}><AdminSchedule reminders={reminders} users={users} settings={settings} scheduleEvents={scheduleEvents} leaveRequests={leaveRequests} /></AdminSectionWrap>;
...
// ── 급여명세서 ──────────────────────────────────────────────────
function PayslipScreen({ user, users, payslips, insuranceSlips = [], reads }) {
  const isAdmin = user.role === "admin";
  const [uploading, setUploading] = useState(false);
  ...
  const [expanded, setExpanded] = useState({});
  const [docTab, setDocTab] = useState("pay");
  const [expandedIns, setExpandedIns] = useState({});
  const kstNow = new Date(new Date().getTime() + 9*60*60*1000);
  ...
  const myInsSlips = (isAdmin ? insuranceSlips : insuranceSlips.filter(s => s.userId === user.id))
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {[["pay", "급여명세서"], ["insurance", "4대보험 내역"]].map(([key, label]) => (
          <button key={key} onClick={() => setDocTab(key)}
            style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: `1px solid ${docTab === key ? T.blue : T.border}`, background: docTab === key ? T.blueBg : T.card, color: docTab === key ? T.blue : T.sub, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
            {label}
          </button>
        ))}
      </div>

      {/* 연도 선택 (급여명세서/4대보험 내역 공통) */}
      ...
      {docTab === "insurance" && (
        myInsSlips.length === 0
          ? <div style={{ textAlign: "center", color: T.muted, padding: 40 }}>등록된 4대보험 내역이 없어요</div>
          : myInsSlips.map(s => {
            const unread = !isAdmin && !reads?.[`${user.id}_insSlip_${s.id}`];
            const isOpen = !!expandedIns[s.id];
            const handleRead = async () => {
              const key = `${user.id}_insSlip_${s.id}`;
              if (!reads?.[key]) await setDoc(doc(db, COL_READS, key), { userId: user.id, type: "insSlip", docId: s.id, readAt: new Date().toISOString() });
            };
            return (
              <div key={s.id} style={{ background: T.card, borderRadius: 14, marginBottom: 10, border: `1px solid ${unread ? T.blue : T.border}`, overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", cursor: "pointer" }}
                  onClick={() => { setExpandedIns(e => ({ ...e, [s.id]: !e[s.id] })); handleRead(); }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      {unread && <div style={{ width: 8, height: 8, borderRadius: "50%", background: T.blue, flexShrink: 0 }} />}
                      <div style={{ fontWeight: unread ? 800 : 700, fontSize: 14, color: T.text }}>
                        {isAdmin && `${s.name} · `}{monthLabel(s.month)}
                      </div>
                    </div>
                    <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>
                      {s.createdAt ? new Date(s.createdAt).toLocaleDateString("ko-KR") : ""} 등록 · 본인부담 {Number(s.합계_근로자 || 0).toLocaleString()}원
                    </div>
                  </div>
                  <span style={{ color: T.muted, fontSize: 14 }}>{isOpen ? "▲" : "▼"}</span>
                </div>
                {isOpen && (
                  <div style={{ background: T.bg, padding: "12px 16px", borderTop: `1px solid ${T.border}` }}>
                    <div style={{ fontSize: 10, color: T.muted, marginBottom: 8 }}>
                      국민연금 기준소득월액: {Number(s.pension || 0).toLocaleString()}원 · 건강/고용 보수월액: {Number(s.health || 0).toLocaleString()}원
                    </div>
                    <div style={{ borderRadius: 10, overflow: "hidden", border: `1px solid ${T.border}` }}>
                      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", background: "#f1f5f9", padding: "7px 10px", fontSize: 10, fontWeight: 700, color: "#475569" }}>
                        <span>항목</span><span style={{ textAlign: "right" }}>본인부담</span><span style={{ textAlign: "right" }}>회사부담</span>
                      </div>
                      {(s.rows || []).map((row, i) => (
                        <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "8px 10px", borderTop: `1px solid ${T.border}`, background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                          <div>
                          ...
                          <div style={{ textAlign: "right", fontSize: 11, fontWeight: 700, color: row.근로자 === 0 ? T.muted : "#dc2626" }}>{Number(row.근로자 || 0).toLocaleString()}</div>
                          <div style={{ textAlign: "right", fontSize: 11, fontWeight: 700, color: row.사업주 === 0 ? T.muted : "#d97706" }}>{Number(row.사업주 || 0).toLocaleString()}</div>
                        </div>
                      ))}
                      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", padding: "10px", borderTop: `2px solid ${T.border}`, background: "#f8fafc" }}>
                        <div style={{ fontSize: 12, fontWeight: 800, color: T.text }}>합계</div>
                        <div style={{ textAlign: "right", fontSize: 12, fontWeight: 800, color: "#dc2626" }}>{Number(s.합계_근로자 || 0).toLocaleString()}원</div>
                        <div style={{ textAlign: "right", fontSize: 12, fontWeight: 800, color: "#d97706" }}>{Number(s.합계_사업주 || 0).toLocaleString()}원</div>
                      </div>
                    </div>
                  </div>
                )}
...
// ── 연차 현황 ────────────────────────────────────────────────────
function AnnualScreen({ user, users, annual, leaveRequests, memberInfo = {}, onBack }) {
  const isAdmin = user.role === "admin";
  const members = users.filter(u => u.role === "member" && (!u.status || u.status === "active"));
  ...
  // 근로기준법 제60조 기준 법정 최소 연차일수 참고 계산 (80% 이상 출근 전제)
  // - 근속 1년 미만: 매월 개근 시 1일 발생 (최대 11일, 자동판정 불가 → 참고만 제공)
  // - 근속 1년 이상: 15일 + (근속연수-1)/2 가산, 25일 한도
  const calcLegalAnnual = (joinDateStr, targetYear) => {
    if (!joinDateStr) return null;
    const join = new Date(joinDateStr);
    const refDate = new Date(targetYear, 0, 1); // 해당 연도 1/1 기준 근속연수
    if (refDate < join) return { years: 0, days: null, note: "해당 연도 이전 입사 전이라 계산 대상 아님" };
    let years = refDate.getFullYear() - join.getFullYear();
    const anniv = new Date(refDate.getFullYear(), join.getMonth(), join.getDate());
    if (refDate < anniv) years--;
    if (years < 1) {
      return { years, days: null, note: "입사 1년 미만: 매월 개근 시 1일 발생(최대 11일) — 근태기록으로 개근여부 확인 필요" };
    }
    const days = Math.min(25, 15 + Math.floor((years - 1) / 2));
    const prorateNote = join.getMonth() === 0 && join.getDate() === 1
      ? ""
      : " (입사일이 1/1이 아니므로, 회계연도 통일 정책 사용 시 입사 첫 해는 비례 부여 등 회사 방식에 따라 달라질 수 있어요)";
    return { years, days, note: `근속 ${years}년 → 법정 최소 ${days}일${prorateNote}` };
  };
          ...
          {members.map(u => {
            ...
            return (
              <div key={u.id} style={{ background: T.card, borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: `1px solid ${T.border}` }}>
                ...
                {editUser === u.id ? (
                  <div>
                    {memberInfo[u.id]?.joinDate && (() => {
                      const thisCalc = calcLegalAnnual(memberInfo[u.id].joinDate, new Date().getFullYear());
                      const nextCalc = calcLegalAnnual(memberInfo[u.id].joinDate, new Date().getFullYear() + 1);
                      return (
                        <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 10, padding: "10px 12px", marginBottom: 10, fontSize: 11, color: "#0369a1", lineHeight: 1.6 }}>
                          📐 법정 연차 참고 (근로기준법 §60, 입사일 {memberInfo[u.id].joinDate})
                          <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 4 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                              <span>{new Date().getFullYear()}년: {thisCalc.note}</span>
                              {thisCalc.days != null && (
                                <button onClick={() => setTotal(thisCalc.days)}
                                  style={{ flexShrink: 0, background: "#0284c7", border: "none", color: "#fff", borderRadius: 6, padding: "3px 9px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>적용</button>
                              )}
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                              <span>{new Date().getFullYear() + 1}년: {nextCalc.note}</span>
                              {nextCalc.days != null && (
                                <button onClick={() => setTotal(nextCalc.days)}
                                  style={{ flexShrink: 0, background: "#0284c7", border: "none", color: "#fff", borderRadius: 6, padding: "3px 9px", fontSize: 10, fontWeight: 700, cursor: "pointer" }}>적용</button>
                              )}
                            </div>
                          </div>
...
// ── 메인 App ───────────────────────────────────────────────────
function App({ users, settings, records, leaves, notices, board, payslips, insuranceSlips = [], annual, leaveRequests, memberInfo, reads, reminders = [], scheduleEvents = [], contracts = [], notiLog = [], riskAssessments = [], riskSubmissions = [], onSaveUsers, onSaveSettings, onSaveRecord, onSaveLeave }) {
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Noto Sans KR',sans-serif", paddingBottom: 80 }}>
      ...
      {tab === "annual" && (
        <>
          ...
          <AnnualScreen user={user} users={users} annual={annual} leaveRequests={leaveRequests} memberInfo={memberInfo} />
        </>
      )}
      {tab === "payslip" && (
        <>
          ...
          <PayslipScreen user={user} users={users} payslips={payslips} insuranceSlips={insuranceSlips} reads={reads} />
        </>
      )}
      {tab === "contract" && (
        <>
      ...
      {tab === "schedule" && (
        <>
          ...
          <div style={{ padding: 16 }}>
            <MemberScheduleCalendar settings={settings} scheduleEvents={scheduleEvents} leaveRequests={leaveRequests} userId={user?.id} />
          </div>
        </>
      )}
      {tab === "education" && (
