...
const APP_VERSION = "v2026.07.13-r8";
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                  {s.attachments.map((f, i) => (
                    <a key={i} href={f.url} target="_blank" rel="noreferrer"
                      style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 9px", borderRadius: 8, background: T.bg, border: `1px solid ${T.border}`, fontSize: 11, color: T.text, textDecoration: "none" }}>
                      {(f.type?.startsWith("image/") || /\.(jpg|jpeg|png|gif|webp)$/i.test(f.name || "")) ? <img src={f.url} style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 4 }} /> : "📎"}
                      <span>{f.name}</span>
                    </a>
                  ))}
                </div>
